...
it('uses expected indexes for tenant and billing query paths', function () {
    ...
    $partialIndex = DB::selectOne(
        "SELECT indexname FROM pg_indexes WHERE tablename = 'time_logs' AND indexname = 'time_logs_unbilled_task_id_index'"
    );

    expect($partialIndex)->not->toBeNull();
});
